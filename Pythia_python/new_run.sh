...
stdbuf -oL python3 -u nhs.py --trace_length ${trace_length} --benchmark ${benchmark} --page_embed_size ${page_embed_size} --pc_embed_size ${pc_embed_size} --offset_embed_size ${offset_embed_size} --offset_embed_size_internal ${offset_embed_size_internal} --lstm_size 256 --keep_ratio ${keep_ratio} --step_size ${step_size} --learning_rate 0.001 --learning_rate_decay ${learning_rate_decay} --complete_loss ${complete_loss} --complete_embedding ${complete_embedding} --batch_size ${batch_size} --use_pc_history ${use_pc_history} --lstm_layer ${lstm_layer} --pc_localization ${pc_localization} --file_loc "$file_loc" 2>&1 &> LOGS.log
