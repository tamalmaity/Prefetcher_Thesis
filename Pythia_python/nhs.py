...
def build_and_train_network(benchmark, args):
    ...
    for ext in workloads: 
        #path = load_trace+ext
        #path = "/home/tamalmaity/data/LoadTraces/spec06/462.libquantum-s0.txt.xz"
            ...
            with lzma.open(file, mode='rt') as f:
                ...
                for line in f:
                    ...
                    if pc not in unique_pcs:
                        unique_pcs[pc] = len(unique_pcs) # dict for storing all unique pcs
    ...
    for keys in rev_sort_offset_jump_dict:
    ...
    for (test_pc, test_addr) in zip(test_pc_in, test_addr_in):
        ...
        if cnt == len(test_addr_in) - 1: break
        
        size += 1
        cnt += 1

        #pline[1]>>12, (pline[1]>>6)&0x3f
        ...
        actual_page = test_addr_in[cnt] >> 12
        actual_offset = (test_addr_in[cnt] >> 6) & 0x3f
    ...
    print("Pythia stats")
    for k in reinforcement.stats:
        print(k, reinforcement.stats[k])
        print('\n')

    print("Learn featurewise stats")
    for k in reinforcement.brain_featurewise.stats:
        print(k, reinforcement.brain_featurewise.stats[k])
